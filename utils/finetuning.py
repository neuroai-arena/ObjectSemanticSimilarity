...
def run_epoch(args, dataloader, network, lin_model, optimizer=None, fabric=None, t=None, epoch=0):
    loss_meter = AverageMeter('loss')
    acc_meter = AverageMeter('acc')
    for i_batch, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc="Epoch " + str(epoch)):
        # print(i_batch)
        with torch.no_grad():
            labels = batch[1+args.fine_label]
            e = network(t(batch[0][0])).detach()


        if args.fine_proj:
            for l in range(len(network.projector.net)):
                e = network.projector.net[l](e)
                if l >= args.fine_proj:
                    break
...
def finetune(args, train_dataloader, train_set, test_dataloader, net, save_dir, fabric, tf, tv):
    # features, labels = get_representations(args, network, train_dataloader, 10000)
    # max_labels = (torch.max(labels) + 1).item()

    num_outputs = net.num_output
    if hasattr(net, "projector") and args.fine_proj:
        if args.fine_proj == len(net.projector.net)-1:
    ...
    # Prepare model
    class_number = train_set.n_classes if not hasattr(train_set, "all_classes") else train_set.all_classes[args.fine_label]
    lin_model = nn.Linear(num_outputs, class_number)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if not args.fine_eval:
            net.train()
        run_epoch(args, train_dataloader, net, lin_model, optimizer, fabric=fabric, t=tf, epoch=epoch)
        if not args.fine_eval:
            net.eval()
